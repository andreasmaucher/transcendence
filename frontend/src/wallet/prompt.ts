...
export function showSaveMatchPrompt(gameState: any, onSave: SaveHandler) {
	...
	if (typeof gameState?.tick === "number" && gameState.tick === lastShownForTick) return;
	...
	saveBtn.onclick = async () => {
		try {
			...
			if (!s.connected || !s.address || !isOnFuji(s.chainId)) {
				status.textContent = "Save blocked: connect wallet to Avalanche Fuji (43113).";
				return;
			}
			...
			const tId =
				...
				null;
			const gId =
				(saveResult && (saveResult as any).gameId) || (gameState && (gameState.gameId || gameState.id)) || null;
			const txHash = (saveResult && (saveResult as any).txHash) ?? null;
			if (tId && gId) {
				try {
					const saved = await fetchSavedMatch(s.provider, String(tId), String(gId));
