...
export default async function userRoutes(fastify: FastifyInstance) {
	...
	fastify.post("/api/users/login", async (request, reply) => {
		...
		reply.header(
			"Set-Cookie",
			makeSessionCookie(token, { secure, maxAgeSec })
		);
	...
	fastify.post("/api/users/register", async (request, reply) => {
		...
		try {
			...
			return reply
				.code(400)
				.send({ success: false, message: "Unable to register user" });
		}
	});
	...
	fastify.post("/api/users/update", async (request, reply) => {
		...
		const updates = [newUsername, password, avatar].filter(
			(v) => v !== undefined
		);
		...
		if (updates.length !== 1)
			return reply
				.code(400)
				.send({ success: false, message: "Too many values to update" });
